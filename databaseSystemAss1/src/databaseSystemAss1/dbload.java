...
public class dbload {
	...
	public static void main(String[] args) {
		...
		System.out.println("Time taken :" + (duration / 1000) + "s");
	}

	public static byte[] combineRecords(byte[] data, byte[] destination) {
		...
		try {
			...
			while ((in = br.readLine()) != null) {
				...
				if ((page + b.length) < pagesize) {
					buffer = combineRecords(b, buffer);
					page += b.length;
				} else {
					byte[] temp = new byte[pagesize];
					System.arraycopy(buffer, 0, temp, 0, buffer.length);
					os.write(temp);
					System.out.print("Writing page " + pageNum + "\r");
					buffer = new byte[0];
					combineRecords(b, buffer);
					page = b.length;
					pageNum++;
				}
			}
			...
			System.out.println("Pages created : " + pageNum);
